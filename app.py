...
def get_ai_answer(question: str) -> str:
    ...
    try:
        ...
        data = {
            "model": "deepseek-chat",
            "messages": [
                {"role": "system", "content": "你是一个专业的用药顾问，回答用药相关问题时要准确、安全、易懂。"},
...
if st.session_state.page == "home":
    ...
    if not valid_medicines:
        ...
        for r in reminders:
            if r["taken"]:
                st.success(f"✅ {r['time']} {r['medicine_name']} - {r['dosage']}（已服用）")
            else:
                col1, col2 = st.columns([4, 1])
                with col1:
                    st.warning(f"⏰ {r['time']} {r['medicine_name']} - {r['dosage']}")
                with col2:
                    if st.button("已服", key=f"take_{r['med_id']}_{r['time']}"):
                        db.mark_taken_by_name(r["medicine_name"], r["time"])
                        st.rerun()
...
# ========== 2. 扫码加药 ==========
    ...
    with tab2:
        ...
        input_mode = st.radio("选择输入方式", ["🔤 药品名称", "🔢 条形码"], horizontal=True, label_visibility="collapsed")
